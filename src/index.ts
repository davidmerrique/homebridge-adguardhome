...
class AdGuardHome implements AccessoryPlugin {
  ...
  private glinetSid: string | undefined;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private glinetSidTimeout: any | undefined;

  private readonly storageName: string;
  private readonly gotInstance: Got | undefined;
    ...
    service
      ...
      .on(
        ...
        (callback: CharacteristicGetCallback) => {
          ...
          callback(
            null,
            this.isActiveAccessory(service) ? this.targetState : this.onState,
          );
        },
      )
                ...
                const run = async () => {
                  ...
                  this.accessoryServices.forEach((service) => {
                    service
                      .getCharacteristic(hap.Characteristic.LockCurrentState)
                      .updateValue(
                        this.isActiveAccessory(service)
                          ? this.currentState
                          : this.onState,
                      );
                  });
                ...
                this.gotInstance("status")
                  ...
                  .then((body: any) => {
                    ...
                    if (this.stateLogging)
                      this.log.info(
                        `Current AdGuard Home state: ${enabled ? "🔒 Locked" : "🔓 Unlocked"}`,
                      );
  ...
  private runTimer(timer: number, fromCheckUnfinishedTimer = false) {
    ...
    this.autoOnHandler = setTimeout(
      () => {
        this.log.info(
          `⏲️ - The ${timer} minute${timer > 1 ? "s" : ""} timer finish`,
        );
  ...
  private isActiveAccessory(accessory) {
    return (
    ...
    if (this.isJammed()) {
      ...
      this.accessoryServices.forEach((accessory) => {
        ...
        if (isActiveAccessory || this.stateLogging)
          this.log(
            `${accessory.displayName} - Current status: ${currentState ? "🔒 Locked" : "🔓 Unlocked"}`,
          );
      });
    }
  }
  ...
  private accessoryIsOffline(error: any) {
    if (!this.isJammed()) {
      if (this.debug) {
        if (error.response)
          this.log.error(`🐞 - Offline - ${error.response.body}`);
    ...
    if (this.isGlinet) {
      ...
      this.gotInstance
        ...
        .then((res: any) => {
          ...
          this.log.info(
            `Setting to: ${status === this.onState ? "🔒 Locked" : "🔓 Unlocked"}`,
          );
  ...
  private async getGlinetEncryption() {
    try {
      ...
      if (response.result) {
        ...
        return hash_value;
      } else throw new Error(`No API Result - ${response.error.message}`);
    } catch (error) {
      if (this.debug)
        this.log.info(`🐞 - Encryption - Disconnected - ${error}`);
    }
  ...
  private async getGlinetSID() {
    if (this.glinetSid) return this.glinetSid;
    ...
    try {
      ...
      } else throw new Error(`No hash value`);
    } catch (error) {
  ...
  private async getGlinetState() {
    try {
      ...
      if (response.result) {
        if (this.debug)
          this.log.info(
            `🐞 - AdGuard: ${this.onOff(response.result.enabled)}, DNS: ${this.onOff(response.result.dns_enabled)}`,
          );

        // Return AdGuard Home state
        return response.result.enabled && response.result.dns_enabled;
      } else throw new Error(`Get - ${response.error.message}`);
    } catch (error) {
