...
class AdGuardHome implements AccessoryPlugin {
  ...
  private readonly gotInstance: any | undefined
  ...
  private assignDefaultEvents(service: Service) {
    service
      ...
      .on(
        CharacteristicEventTypes.GET, (callback: CharacteristicGetCallback) => {
          ...
          (callback: CharacteristicGetCallback) => {
            ...
            if (!this.onAPICall) {
              ...
              if (this.isGlinet) {
                // Glinet stuff
                let run = async () => {
                  let status = await this.getGlinetState();
  ...
  private isActiveAccessory(accessory: any) {
    return (accessory.subtype?.includes("main_") || (this.activeAutoOnTimer != undefined && accessory.subtype == this.activeAutoOnTimer))
  }
  ...
  private updateStates() {
    if (this.isJammed()) {
      ...
      this.accessoryServices.forEach((accessory) => {
        ...
        if (isActiveAccessory || this.stateLogging) this.log(`${accessory.displayName} - Current status: ${currentState ? "🔒 Locked" : "🔓 Unlocked"}`)
      })
    }
  }
  ...
  private async getGlinetSID() {
    ...
    try {
      let hash_value = await this.getGlinetEncryption()
    ...
    try {
      ...
      const response: any = await got.post(this.rpc, {
        json: {
          jsonrpc: "2.0",
          method: "call",
          params: [sid, "adguardhome", "set_config", {
            "enabled": true,
            "dns_enabled": state
          }],
          id: 0
